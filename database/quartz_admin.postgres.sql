...
(
  ...
  CONSTRAINT tbl_instances_pkey PRIMARY KEY (instanceid)
)

CREATE TABLE tbl_instanceproperties
(
  instancepropertyid serial NOT NULL,
  propertyname text,
  propertyvalue text,
  instanceid integer NOT NULL,
  CONSTRAINT tbl_instanceproperties_pkey PRIMARY KEY (instancepropertyid),
  CONSTRAINT fk_tbl_instanceproperties_instanceid_tbl_instances_instanceid FOREIGN KEY (instanceid)
      REFERENCES tbl_instances (instanceid) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
)

-- http://dev.nauck-it.de/projects/1/wiki/Install
